...
router.post('/send', authMiddleware, async (req, res) => {
  try {
    ...
    const complianceCheck = await ComplianceService.checkTransactionCompliance(senderId, amountInCents, 'p2p_transfer');
    ...
    // AML Monitoring
    await ComplianceService.monitorTransaction({
      userId: senderId,
...
router.post('/bank-transfer', authMiddleware, async (req, res) => {
  try {
    ...
    // Compliance Check
    const amountInCents = Math.round(amount * 100);
    const complianceCheck = await ComplianceService.checkTransactionCompliance(userId, amountInCents, 'bank_transfer');
    if (!complianceCheck.allowed) {
      return res.status(403).json({ 
        error: 'Transaction not allowed', 
        reason: complianceCheck.reason 
